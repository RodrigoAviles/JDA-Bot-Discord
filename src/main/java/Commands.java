...
public class Commands extends ListenerAdapter implements Searcher {
    ...
    Comando repeat = new Comando("repeat");
    LinkKey linkKeyMovies = new LinkKey("https://www.imdb.com/list/ls055386972/","n/a");
    LinkKey linkKeyMeme= new LinkKey("https://meme-api.herokuapp.com/gimme","n/a");
    LinkKey linkKeyMovie = new LinkKey("http://www.omdbapi.com/?t=","&apikey=9695e938");
    LinkKey linkKeyDog=new LinkKey("https://dog.ceo/api/breeds/image/random","n/a");
    LinkKey linkKeyChuck=new LinkKey("https://api.chucknorris.io/jokes/random","n/a");
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if (args[0].equalsIgnoreCase(prefix + movie.getComando())) {//esto capta el commando no es case sensitive
            ...
            try {
                ...
                System.out.println("la pelciula fallo codigo Plus Exception");
            }
        ...
        if (args[0].equalsIgnoreCase(prefix + topMovies.getComando())) {
            ...
            TopMovies movies = new TopMovies(linkKeyMovies.getLink());
        ...
        if (args[0].equalsIgnoreCase(prefix + meme.getComando())) {
            ...
            Conectador conector = new Conectador(linkKeyMeme.getLink());
        ...
        if (args[0].equalsIgnoreCase(prefix + repeat.getComando())) {
            try {
                event.getChannel().sendMessage(VozComando(args)).tts(true).queue();
                event.getMessage().delete().queue();
            }catch (Exception e){
                System.out.println("el !repeat fallo debido al codigo 404");
            }
            }
    ...
    public EmbedBuilder ChuckComando(String[] args) {
        ...
        Conectador conector = new Conectador(linkKeyChuck.getLink());
        EmbedBuilder embed = new EmbedBuilder();
        try {
    ...
    public EmbedBuilder DogComando(String[] args) {
        ...
        Conectador conector = new Conectador(linkKeyDog.getLink());
        try {
            String line = conector.Conectar();
            JSONParser parser = new JSONParser();
    ...
    public EmbedBuilder MovieComando(String[] args) {
        ...
        pelicula = Frase(args);
        EmbedBuilder embed = new EmbedBuilder();
        Conectador conector = new Conectador(linkKeyMovie.getLink() + pelicula + linkKeyMovie.getKey());
        try {
            String line = conector.Conectar();
            JSONParser parser = new JSONParser();
    ...
    public EmbedBuilder HelpComando() {
        ...
        comandosA.add(dog);
        comandosA.add(movie);
        comandosA.add(topMovies);
        comandosA.add(repeat);
